...
void ILI9341_Draw_Char(char Character, uint8_t X, uint8_t Y, uint16_t Colour, uint16_t Size, uint16_t Background_Colour) 
{
 ...
 //  ILI9341_Draw_Rectangle(X, Y, CHAR_WIDTH*Size, CHAR_HEIGHT*Size, Background_Colour);
    ...
    ILI9341_Set_Address(X, Y, X+CHAR_WIDTH*Size-1, Y+CHAR_HEIGHT*Size-1);
    ...
    for(i = 0; i < font.height; i++) {
        ...
        for(j = 0; j < font.width; j++) {
            if((b << j) & 0x8000)  {
                ...
                spi_write(&HSPI_INSTANCE, (unsigned char*)data,sizeof(data));
            } else {
...
void ILI9341_Image(const char* Image_Array,uint8_t x,uint8_t y,uint8_t width,uint8_t height, uint8_t Orientation)
{
    width=width-1;
    height=height-1;
    if(Orientation == SCREEN_HORIZONTAL_1)
    {
        ILI9341_Set_Rotation(SCREEN_HORIZONTAL_1);
        ILI9341_Set_Address(x,y,x+width,y+height);
    ...
    {
        ILI9341_Set_Rotation(SCREEN_VERTICAL_2);
        ILI9341_Set_Address(0,0,height,width);
        ...
        unsigned char Temp_small_buffer[BURST_MAX_SIZE];
        uint32_t counter = 0;
        for(uint32_t i = x; i < width*height*2/BURST_MAX_SIZE; i++)
        {
            for(uint32_t k = y; k< BURST_MAX_SIZE; k++)
            {
                Temp_small_buffer[k]	= Image_Array[counter+k];
            }
